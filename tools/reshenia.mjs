...
/*
 * Шаг мира. Ставится переменной среды, потому что 1/60 — это допущение о
 * частоте кадров у игрока, а не свойство игры. Владелец играет с
 * телефона, где кадров бывает тридцать, и если числа при 1/30 расходятся
 * с числами при 1/60, значит игра зависит от частоты кадров — а это
 * поломка, которую на своём мониторе не видно никогда.
 *
 *     SHAG=30 node tools/reshenia.mjs 60 10,12
 *
 * Имя переменной латиницей нарочно: zsh не принимает кириллицу в имени
 * переменной среды и отвечает «command not found» на саму строку.
 */
const DT = 1 / Number(process.env.SHAG || 60);

/*
 * Вероятности действий бота заданы НА КАДР, а сравниваем мы разные шаги
 * мира. Без поправки при тридцати кадрах бот действует вдвое реже, чем
 * при шестидесяти, — и расхождение чисел описывает бота, а не игру.
 *
 * Поймано сразу: без поправки выживаемость шла 2 / 10 / 13 из сорока при
 * 30 / 60 / 120 кадрах, и это выглядело как «игра зависит от частоты
 * кадров». Половина разницы оказалась моей.
 */
const ТЕМП = DT * 60;
...
function прогон(floor, seed, откуда, секунд = 60, толково = true, контроль = false) {
  ...
  try {
    ...
    const когда = [];
    /* Тревога — выключатель тихой половины игры, и он односторонний.
       Меряем, доживает ли тишина хоть до середины прогона. */
    let тревогаНа = -1;
    ...
    for (let f = 0; f < секунд / DT && world.player.alive; f += 1) {
      ...
      if (world.player.chargeLeft <= 0) {
        if (набор.length < 3 && rnd() < 0.06 * ТЕМП) {
          const id = world.elements[Math.floor(rnd() * world.elements.length)];
          intent.charge = id;
          набор.push(id);
        } else if (набор.length && цель.length && rnd() < 0.08 * ТЕМП) {
          /*
       ...
       * что он там сделает.
       */
      if (толково && rnd() < 0.05 * ТЕМП && цель.length) {
      ...
      for (const event of world.events) {
        ...
        if (event.type === 'slow') замедлений += 1;
        if (event.type === 'engaged' && тревогаНа < 0) тревогаНа = f * DT;
    ...
    if (тревогаНа >= 0) тревоги.push(тревогаНа); else тихих += 1;
    for (const rule of trace.rules) поПравилам.set(rule, (поПравилам.get(rule) || 0) + 1);
...
/*
 * Этаж выбирается переменной среды. Без этого замер отвечал только про
 * обучалку, где выдают огонь и молнию, — а половина правил мира требует
 * ветра, воды или земли и на ней недостижима в принципе.
 *
 *     ETAZH=2 node tools/reshenia.mjs 60
 */
const floor = CAMPAIGN[Number(process.env.ETAZH || 0)] || CAMPAIGN[0];
...
const тревоги = [];
let тихих = 0;
/* Сколько прогонов встретили каждое правило. Ноль тут важнее любого
   среднего: правило, которое не встретилось ни разу, в игре как бы и
   нет, сколько бы кода на него ни было написано. */
...
if (тревоги.length) {
  тревоги.sort((a, b) => a - b);
  console.log(`тревога поднялась в ${тревоги.length} прогонах из ${сколько}, `
    + `тихих до конца ${тихих}; медиана ${тревоги[тревоги.length >> 1].toFixed(1)} с, `
    + `самая ранняя ${тревоги[0].toFixed(1)} с`);
} else {
  console.log(`тревога не поднималась ни разу за ${сколько} прогонов`);
}
{
  const строки = [...поПравилам.entries()].sort((a, b) => b[1] - a[1]);
  console.log('встречи правил: ' + (строки.length
    ? строки.map(([r, n]) => `${r} ${n}`).join(', ') : 'ни одного'));
}
console.log(`замедлений времени: ${замедлений} за ${сколько} прогонов `
  + `(${(замедлений / сколько).toFixed(2)} на прогон в минуту)`);
if (размахи.length) {
