...
export function createInput(surface) {
  ...
  surface.addEventListener('mousedown', (event) => {
    /* Клик — это и «бей», и «вот туда»: место нажатия запоминается тем же
       способом, что и тап пальцем, и разбирается уже в игре. */
    if (event.button === 0) {
      mouse.down = true;
      pressed.add('Fire');
      const rect = surface.getBoundingClientRect();
      tap = { x: event.clientX - rect.left, y: event.clientY - rect.top };
    }
