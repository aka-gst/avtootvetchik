...
/* --- Выход с партии: площадь нажатия и вопрос --- */
{
  /*
   * Обе вещи живут в файлах, которые нельзя импортировать в узле, —
   * разметка и стили. Читаем текстом: это слабее прогона, но ловит
   * удаление и правку по индексам, а именно так они и пропадают.
   *
   * Числа с бою, 390x844: было 54x19, стало 60x44. Девятнадцать точек —
   * меньше трёх миллиметров.
   */
  const css = readFileSync(new URL('../style.css', import.meta.url), 'utf8');
  const главный = css.slice(css.indexOf('.game-home-menu {'));
  const блок = главный.slice(0, главный.indexOf('}'));
  check('выход: область нажатия не меньше 44 по обеим сторонам',
    /min-height:\s*44px/.test(блок) && /min-width:\s*44px/.test(блок));

  const main = readFileSync(new URL('../src/main.js', import.meta.url), 'utf8');
  check('выход: при начатой партии спрашивает', main.includes('window.confirm('));
  check('выход: спрашивает только при начатой партии',
    main.includes('if (!партияИдёт()) return;'));

  /*
   * Порядок важнее наличия: `preventDefault` обязан стоять ДО вопроса.
   * Если он после, «отмена» не удержит адрес — браузер уже ушёл по
   * ссылке, и проверка, знающая только исход «выйти», будет зелёной на
   * сломанном.
   */
  const стоп = main.indexOf('event.preventDefault()');
  const вопрос = main.indexOf('window.confirm(');
  check('выход: preventDefault стоит до вопроса, а не после',
    стоп > 0 && вопрос > стоп);
}
